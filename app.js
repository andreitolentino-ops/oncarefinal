...
// -------------------- App state --------------------
// -------------------- Auto logoff after 1 minute of inactivity --------------------
let autoLogoffTimer = null;
function resetAutoLogoffTimer() {
  if (autoLogoffTimer) clearTimeout(autoLogoffTimer);
  autoLogoffTimer = setTimeout(() => {
...
['mousemove','keydown','mousedown','touchstart'].forEach(evt => {
  window.addEventListener(evt, resetAutoLogoffTimer);
});
window.addEventListener('load', resetAutoLogoffTimer);
...
function applyRoleUI(){
  ...
  // Hide most clinical tabs from admission and bednav - they only see dashboard, departments, and basic patient info
    ...
    const tabAccess = {
      admin: [
        'tab-dashboard', 'tab-departments', 'tab-info', 'tab-id', 'tab-history', 'tab-physical', 'tab-assessment',
        'tab-labs', 'tab-meds', 'tab-messages', 'tab-vitals', 'tab-nurse', 'tab-doctor', 'tab-admission', 'tab-plan', 'tab-staff'
      ],
      doctor: [
        'tab-dashboard', 'tab-departments', 'tab-info', 'tab-id', 'tab-history', 'tab-physical', 'tab-assessment',
        'tab-labs', 'tab-meds', 'tab-messages', 'tab-vitals', 'tab-doctor', 'tab-nurse', 'tab-admission', 'tab-plan', 'tab-staff'
      ],
      nurse: [
        'tab-dashboard', 'tab-departments', 'tab-info', 'tab-id', 'tab-history', 'tab-physical', 'tab-vitals',
        'tab-meds', 'tab-messages', 'tab-nurse', 'tab-admission', 'tab-plan', 'tab-staff'
      ],
      admission: ['tab-dashboard', 'tab-departments', 'tab-info', 'tab-admission', 'tab-plan'],
      bednav: ['tab-dashboard', 'tab-departments', 'tab-info'],
      guest: ['tab-dashboard', 'tab-departments', 'tab-info']
    };
    ...
    allTabIds.forEach(tabId => {
      ...
      allTabBtns.forEach(btn => { if(btn) btn.style.display = 'none'; });
      if(tabPanel) tabPanel.classList.add('hidden');
    });

    // Show only allowed tabs for current role
    (tabAccess[currentRole] || tabAccess['guest']).forEach(tabId => {
      const allTabBtns = document.querySelectorAll(`[data-target="${tabId}"]`);
      const tabPanel = document.getElementById(tabId);
      allTabBtns.forEach(btn => { if(btn) btn.style.display = ''; });
